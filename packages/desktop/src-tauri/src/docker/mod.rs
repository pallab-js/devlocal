...
pub async fn list_containers(docker: State<'_, DockerState>) -> Result<Vec<ContainerInfo>, String> {
    ...
    docker_ops::list_containers(docker)
        .await
...
pub async fn start_container(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::start_container(docker, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn stop_container(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::stop_container(docker, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn restart_container(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::restart_container(docker, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn inspect_container(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::inspect_container(docker, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_network_topology(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::get_network_topology(docker)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn inspect_network(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::inspect_network(docker, &id)
        .await
        .map_err(|e| e.to_string())
}
...
/// Start streaming Docker events — aborts any previous stream first.
#[tauri::command]
...
/// Get per-container CPU and memory stats.
#[tauri::command]
pub async fn get_container_stats(
    ...
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::get_container_stats(docker, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn list_volumes(docker: State<'_, DockerState>) -> Result<Vec<VolumeInfo>, String> {
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::list_volumes(docker)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn prune_volumes(docker: State<'_, DockerState>) -> Result<u64, String> {
    let docker = docker.0.as_ref().ok_or("Docker not connected")?;
    docker_ops::prune_volumes(docker)
        .await
        .map_err(|e| e.to_string())
}
